 101.3 Change runlevels / boot targets and shutdown or reboot the system · lpic1book   window.dataLayer = window.dataLayer || \[\]; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'G-4CZV14VZXW'); 
...
It is also possible to _isolate_ any of the targets or move to two special targets too:

1.  `rescue`: Local file systems are mounted, no networking, and root-user only (_maintenance_ mode)
2.  `emergency`: Only the root file system and in read-only mode, No networking and root-user only(_maintenance_ mode)
3.  `reboot`
...
#
# Author:       Miquel van Smoorenburg, <[[email protected]](/cdn-cgi/l/email-protection)>
...
This subsystem lets OS commands (like shutdown) send signals to the computer which results in powering down of the whole PC. In older times we used to have these mechanical keyboards to do a _real_ power down after the OS has done its shutdown and told us that "it is not safe to power down your computer".
...
* `wall`: Sending _wall messages_ to logged-in users
