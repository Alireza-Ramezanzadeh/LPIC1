# [101.2 Boot the System](./1012-boot-the-system.html)
...
function validateForm(query) { return (query.length > 0); } (function () { if (window.location.hash.match(/^#comment-\\d+$/)) { $('#comment\_thread').collapse('show'); } })(); window.onhashchange=function(){ if (window.location.hash.match(/^#comment-\\d+$/)) window.location.reload(true); } $('#comment\_thread').on('shown', function () { var link = document.getElementById('comment-accordion-toggle'); var old\_innerHTML = link.innerHTML; $(link).fadeOut(200, function() { $(this).text('Click here to hide comments').fadeIn(200); }); $('#comment\_thread').on('hidden', function () { $(link).fadeOut(200, function() { $(this).text(old\_innerHTML).fadeIn(200); }); }) })
